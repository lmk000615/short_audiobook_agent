...
| 项目 | 值 |
|------|-----|
| **服务地址（基础）** | `http://10.50.121.102:8006`（原 wrapper，无音色克隆） |
| **服务地址（克隆）** | `http://10.50.121.102:8010`（**NEW**；扩展自 `api_server_s2pro_8010.py`，支持音色克隆） |
| **模型** | Fish Audio S2-Pro |
| **采样率** | 44100 Hz |
| **架构** | Dual-Autoregressive (Dual-AR) |
| **显存需求** | ~11GB+ |
| **音色克隆** | **支持**（仅 8010 端口；需 `reference_audio` + `prompt_text`，详见第 15 / 20 节） |
...
**8010 端口已扩展支持音色克隆**（`api_server_s2pro_8010.py`）。原 8006 端口仍保留基础合成行为不变。两端口详细对比见 20.4 节。

| 端口 | 音色克隆 | 多 speaker 显式音色 | 实现文件 |
|---|---|---|---|
| **8006**（原 wrapper） | ❌ 未暴露 | ❌ 未暴露 | `servers/api_server_s2pro.py` |
| **8010**（**NEW** 扩展版） | ✅ multipart `reference_audio` + `prompt_text` | ❌（仍要靠 reference_audio 路径解决） | `servers/api_server_s2pro_8010.py` |

底层 fish-speech 源码已实现 reference audio 支持（见 [fishaudio/fish-speech](https://github.com/fishaudio/fish-speech)）。8010 把它透传出来，详见第 20 节。
...
| 维度 | 云 API `api.fish.audio/v1/tts` | 自托管 `8006`（原 wrapper） | 自托管 `8010`（**NEW** 音色克隆版） |
|---|---|---|---|
| 鉴权 | Bearer token | 无（内网） | 无（内网） |
| 协议 | JSON body | multipart/form-data | multipart/form-data |
| **音色克隆** | `reference_id` 字符串 / `references` 数组 | **未暴露** | **✅ 支持** `reference_audio` + `prompt_text` |
| 多说话人 + per-speaker 音色 | `reference_id: [id1, id2]` 数组 + `<\|speaker:N\|>` | 仅 `<\|speaker:N\|>`（音色随机） | 仅 `<\|speaker:N\|>`（音色随机，仍要靠 `reference_audio` 路径解决） |
| 内联标签 | `[tag]` 全支持 | 同 | 同 |
| 全局 instruction | 支持 | 支持 | 支持 |
| 输出格式 | wav / pcm / mp3 / opus | 仅 wav | 仅 wav |
| 采样率 | 8/16/24/32/44.1/48 kHz | 固定 44.1 kHz | 固定 44.1 kHz |
| Prosody 控制 | `prosody: {speed, volume, normalize_loudness}` | 未暴露 | 未暴露 |
| Latency 模式 | normal / balanced / low | 未暴露 | 未暴露 |
| Chunk 控制 | `chunk_length` / `min_chunk_length` / `condition_on_previous_chunks` | 未暴露 | 未暴露 |
| Repetition penalty | 支持 | 未暴露 | 未暴露 |
| 计费 | 按 character 计费 | 免费（自托管） | 免费（自托管） |
| 模型版本 | 始终最新（s2-pro） | 部署版本（s2-pro 4B） | 部署版本（s2-pro 4B） |

**何时用哪个**：
- 单段测试 / 不需要克隆 → 8006（原 wrapper）
- **多角色有声书 / 需要稳定音色 → 8010**（新 wrapper，src_next 默认走这个）
- 需要云独有功能（多 speaker per-speaker id 数组、低延迟直播、mp3 直出） → 云 API
...
| 缺失字段 | 8006 | **8010** | 云 API |
|---|---|---|---|
| `reference_audio` 三件套 | ❌ | ✅ | ✅ |
| `reference_id` 数组（多 speaker 显式音色） | ❌ | ❌ | ✅ |
| `format` / `sample_rate`（mp3 直出） | ❌ | ❌ | ✅ |
| `prosody` 字段（全局韵律） | ❌ | ❌ | ✅ |
| `latency` 模式 | ❌ | ❌ | ✅ |
| `chunk_length` / `condition_on_previous_chunks` | ❌ | ❌ | ✅ |

### 19.2 v2 扩展路线图

| 版本 | 目标 | 状态 |
|---|---|---|
| v1（8006） | 基础合成 + 内联标签 + 全局 instruction | ✅ 已就绪 |
| **v1.5（8010）** | **透传 `reference_audio` 三件套** | **✅ 已就绪（src_next 默认走此端口）** |
| v2.0 | 透传 `prosody` / `chunk_length` / `format` | ⏳ 待开发 |
| v2.5 | 多 speaker + `reference_id` 数组 | ⏳ 待开发 |
| v3.0 | reference_audio 编码缓存 | ⏳ 待开发 |
...
## 20. 自托管 8010 端口完整文档（音色克隆版）

`api_server_s2pro_8010.py` 是基于原 8006 wrapper 扩展的新版本，**唯一新增能力是音色克隆**。其他行为（API 路径、响应格式、采样率）保持一致。

### 20.1 启动 / 停止

```bash
# 启动（与原 wrapper 独立，不冲突）
source ~/miniconda3/bin/activate s2-pro-clean
export LD_LIBRARY_PATH=$HOME/cuda-12.9/lib64:$LD_LIBRARY_PATH
nohup python api_server_s2pro_8010.py \
    --model-path ./s2-pro \
    --host 0.0.0.0 --port 8010 \
    --device cuda:0 \
    > logs/s2pro_8010.log 2>&1 &

# 健康检查
curl --noproxy '*' http://10.50.121.102:8010/health
# 期望：{"status":"ok","model_loaded":true,"device":"cuda:0","sampling_rate":44100}

# 停止
pkill -f api_server_s2pro_8010
```

### 20.2 API 字段（multipart/form-data）

`POST /v1/voicegen/generate`

| 字段 | 类型 | 必填 | 默认 | 说明 |
|---|---|---|---|---|
| `text` | string | ✅ | - | 要合成的文本，支持 `[tag]` 内联标签 |
| `instruction` | string | ❌ | `""` | 全局风格指令（如 `[whisper]`、`温柔亲切的女声`） |
| `temperature` | float | ❌ | `1.0` | 采样温度 (0, 2) |
| `top_p` | float | ❌ | `0.6` | Nucleus 采样 cutoff (0, 1) |
| `max_new_tokens` | int | ❌ | `4096` | 最大生成 token 数（控制音频长度） |
| `reference_audio` | file | ❌ | - | **NEW** 参考音频文件（wav/mp3），建议 10-30 秒干净单语音；≤ 10 MB |
| `prompt_text` | string | 条件必填 | `""` | **NEW** `reference_audio` 的精确转录文本（传了 reference_audio 时必填，必须与 wav 内容**逐字匹配**） |
| `enable_reference_audio` | bool | ❌ | `false` | **NEW** 强制启用克隆模式（不传 reference_audio 时报 400） |

**响应**：wav 字节流（`audio/wav`，44100 Hz / 16-bit mono），含 headers：
- `X-Request-Id`：8 位 hex 请求 ID
- `X-Inference-Time`：处理耗时（秒）
- `X-Voice-Clone`：`true` / `false`（本次是否走了克隆路径）
...
```bash
# 准备：先有一段 10-30 秒的参考音频 + 它的精确转录
# 假设 /tmp/ref.wav 是某人念 "你好，欢迎使用语音设计功能，这是一个测试句子。" 的录音

curl --noproxy '*' -X POST http://10.50.121.102:8010/v1/voicegen/generate \
  -F "text=今天天气真不错，我们一起去公园散步吧。" \
  -F "instruction=温柔亲切的女声" \
  -F "reference_audio=@/tmp/ref.wav" \
  -F "prompt_text=你好，欢迎使用语音设计功能，这是一个测试句子。" \
  -F "temperature=0.7" \
  -F "top_p=0.7" \
  -o /tmp/output.wav

# 验证
file /tmp/output.wav  # RIFF WAVE
ffplay /tmp/output.wav  # 试听：应该音色接近 /tmp/ref.wav
```
...
REF_WAV = "/tmp/ref.wav"
REF_TEXT = "你好，欢迎使用语音设计功能，这是一个测试句子。"

with open(REF_WAV, "rb") as f:
    response = requests.post(
        "http://10.50.121.102:8010/v1/voicegen/generate",
        data={
            "text": "今天天气真不错，我们一起去公园散步吧。",
            "instruction": "温柔亲切的女声",
...
print(f"voice_clone: {response.headers.get('X-Voice-Clone')}")
print(f"inference_time: {response.headers.get('X-Inference-Time')}")
...
curl --noproxy '*' -X POST http://10.50.121.102:8010/v1/voicegen/generate \
  -F "text=测试一下基础合成。" \
  -o /tmp/test.wav
# 不传 reference_audio，走模型默认音色路径，行为同 8006
```

### 20.4 8006 vs 8010 端口对比

| 维度 | 8006（原 wrapper） | 8010（**NEW** 克隆版） |
|---|---|---|
| 实现文件 | `servers/api_server_s2pro.py` | `servers/api_server_s2pro_8010.py` |
| API 路径 | `/v1/voicegen/generate` | 同 |
| 协议 | multipart/form-data | 同 |
| 现有字段（text/instruction/...） | ✅ | ✅ 完全兼容 |
| **音色克隆** | ❌ 未暴露 | **✅ 支持** |
| 多 speaker 显式音色（reference_id 数组） | ❌ | ❌（仍要靠 reference_audio 解决） |
| 响应格式 | wav 字节流 | 同 + 新 headers |
| 性能开销 | 低（无编码步骤） | 高（每次调用编码参考音频，~0.5-2s 额外） |
| 推荐用途 | 单段测试 / 不需克隆 | **多角色有声书 / 需要稳定音色** |

### 20.5 错误码

| HTTP | 触发条件 | 错误消息 |
|---|---|---|
| 400 | 传了 `reference_audio` 但没传 `prompt_text` | `prompt_text is required when reference_audio is provided` |
| 400 | `enable_reference_audio=true` 但没传 `reference_audio` | `enable_reference_audio=true but no reference_audio file provided` |
| 400 | `text` 为空 | `text cannot be empty` |
| 413 | `reference_audio` > 10 MB | `reference_audio too large: N bytes > 10485760` |
| 500 | `reference_audio` 编码失败（损坏 / 不支持格式） | `reference_audio encoding failed: <详情>` |
| 500 | 推理失败 | `Inference error: <详情>` |
| 503 | 模型未加载 | `Model not loaded` |

### 20.6 内部实现要点（供二次开发参考）

1. **`encode_reference_audio(audio_bytes, original_filename) -> torch.Tensor`**：用 DAC codec 把参考音频编码为 VQ tokens，shape `(num_codebooks, T)`
2. **`run_inference(...)`**：monkey-patch 了 `generate_long` 的 `bool(tensor)` 问题（用 `numel() > 0` 替代）
3. **`infer_lock`**：全局推理锁，避免并发请求抢占 GPU（多线程并发时实际是串行推理）
4. **临时文件**：`encode_reference_audio` 内部用 `tempfile.NamedTemporaryFile` 写盘 → 编码 → `unlink`，确保不留垃圾

### 20.7 与 src_next pipeline 的集成

src_next 默认走 8010 端口。配置在 `src_next/profiles/yellow_gemma_qwen_s2pro.yaml`：

```yaml
tts:
  backend: s2pro_http
  base_url: "http://10.50.121.102:8010"   # ← 注意是 8010，不是 8006
  extra_args:
    enable_reference_audio: true            # 启用克隆
    reference_text: "..."                   # 必须与 voicebank.reference_text 完全一致
    max_workers: 4                          # 并发线程
    # ... 其他字段
```

adapter 调用流程（`src_next/tts/s2pro_adapter.py`）：
1. 从 `voicebank_result.speaker_to_voice[speaker]` 查到该角色的 wav 路径
2. multipart 上传 wav + `prompt_text` + `enable_reference_audio=true` 到 8010
3. server 用 DAC codec 编码 wav → VQ tokens → 喂给 S2Pro 模型
4. 返回 wav 字节流，adapter 写到 `<task_dir>/audio_segments/<seg>.wav`
5. 失败段不阻断其他段；缓存命中（wav 已存在）自动跳过 HTTP