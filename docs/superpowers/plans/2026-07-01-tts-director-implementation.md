...
**架构**：
- LLM 按 segment 选 model（约束：同 speaker 用同 model）
...
- system prompt 注入 available_models + 同 speaker 一致性约束 +
  LLM 决策原则
...
- 新增：`tests/test_tts_director_unit.py`（这里起步，任务 4-5 续写）
...
class TTSDirectorAgent:
    ...
    def direct(
        ...
        # 同 speaker 一致性后处理
        self._enforce_same_speaker_consistency(instructions_by_id, segments)
    ...
    def _enforce_same_speaker_consistency(
        self,
        instructions_by_id: dict[str, ModelSpecificTTSInstruction],
        segments: list[Segment],
    ) -> None:
        """如果某 speaker 在输出中有多个 model，统一为首次出现的 model。

        原地修改 `instructions_by_id`。
    ...
    def _apply_fallback(
        ...
        任务 5 实现完整 fallback。当前 stub 按原 segment 顺序返回。
        """
        # 任务 5 会实现完整 fallback
...
- 同 speaker 一致性 + 完整 fallback 留给任务 4-5（stub）
...
### 任务 4：TTSDirectorAgent 同 speaker 一致性

**文件**：
- 修改：`src_next/analysis/tts_director.py`（替换 `_enforce_same_speaker_consistency` stub）
- 修改：`tests/test_tts_director_unit.py`（追加测试）
...
```python
def test_same_speaker_gets_same_model(
    mock_llm, sample_segments, sample_characters, sample_voicebank_result, model_configs_all
):
    """如果 LLM 给同一 speaker 返回了不同 model，后处理统一为首次出现的 model，
    并写 warning。"""
    # sample_segments 中 seg_002 的 speaker 是 "小松鼠"
    # 加一个同 speaker 的 segment 测试跨段一致性
    from src_next.core.data_models import Segment
    extra_segments = sample_segments + [
        Segment(
            segment_id="seg_004",
            segment_type="dialogue",
            speaker="小松鼠",  # 与 seg_002 同 speaker
            text="我们一起玩吧！",
        )
    ]
    # LLM "错误地" 给 seg_002 选 S2Pro，但给 seg_004 选 IndexTTS2
    # （同 speaker 不同 model——应被统一）
    llm_response = {
        "instructions": [
            {"segment_id": "seg_001", "model": "CosyVoice3", "parameters": {}},
            {"segment_id": "seg_002", "model": "S2Pro", "parameters": {"inline_tags_text": "[excited]..."}},
            {"segment_id": "seg_003", "model": "IndexTTS2", "parameters": {}},
            {"segment_id": "seg_004", "model": "IndexTTS2", "parameters": {}},  # 与 seg_002 冲突
        ]
    }
    agent = TTSDirectorAgent(llm_client=mock_llm(llm_response), available_models=list(model_configs_all.values()))
    result = agent.direct(
    ...
    by_id = {inst.segment_id: inst for inst in result}
    # seg_002 和 seg_004 应该有相同的 model（首次出现 = S2Pro）
    assert by_id["seg_002"].model == by_id["seg_004"].model, (
        f"同 speaker (小松鼠) 应有相同 model，实际 "
        f"{by_id['seg_002'].model} vs {by_id['seg_004'].model}"
    )
    assert by_id["seg_002"].model == "S2Pro"  # 首次出现的胜出
```
...
运行：
```bash
pytest tests/test_tts_director_unit.py::test_same_speaker_gets_same_model -v
```

预期：FAIL —— `by_id["seg_002"].model == "S2Pro"` 但 `by_id["seg_004"].model == "IndexTTS2"`（stub 没做一致性）。

- [ ] **步骤 3：替换 stub 实现**

在 `src_next/analysis/tts_director.py`，把 `_enforce_same_speaker_consistency` 的 stub 替换为：
...
```python
    def _enforce_same_speaker_consistency(
        ...
        原地修改 `instructions_by_id`：任何 speaker 在前面已经出现过不同 model
        的 instruction，都被改为首次出现的 model。同时写 warning 让不一致
        可见（通常说明 prompt 需要加强）。

        原因：同 speaker 用不同 TTS backend = 跨段音色不一致，最终音频会
        听起来很违和。
        """
        seg_id_to_speaker = {s.segment_id: s.speaker for s in segments}
        # 按 segment 原顺序确定"首次出现"
        ordered_seg_ids = [s.segment_id for s in segments]

        first_model_per_speaker: dict[str, str] = {}
        for seg_id in ordered_seg_ids:
            inst = instructions_by_id.get(seg_id)
            if inst is None:
                continue
            speaker = seg_id_to_speaker[seg_id]
            if speaker in first_model_per_speaker:
                first_model = first_model_per_speaker[speaker]
                if inst.model != first_model:
                    logger.warning(
                        "同 speaker 一致性：segment %r speaker %r 原本是 model %r，"
                        "统一为首次出现的 %r。（prompt 可能需要加强约束。）",
                        seg_id, speaker, inst.model, first_model,
                    )
                    inst.model = first_model
            else:
                first_model_per_speaker[speaker] = inst.model
```
...
```bash
git add src_next/analysis/tts_director.py tests/test_tts_director_unit.py
git commit -m "feat(tts_director): C1 step 5 — 同 speaker 一致性后处理

LLM 输出后扫描，若同 speaker 出现多个 model，统一为首次出现的 model。
写 warning 到 log（提示 prompt 可能需要加强约束）。

新增测试：4 segments（其中 seg_002 + seg_004 同 speaker 但 LLM 给了不同
model），验证后处理统一后两者 model 一致。
...
### 任务 5：TTSDirectorAgent fallback + parameters 清洗
...
```bash
git add src_next/analysis/tts_director.py tests/test_tts_director_unit.py
git commit -m "feat(tts_director): C1 step 6 — fallback + parameters 清洗
...
def test_tts_director_with_real_gemma4(integration_segments, integration_characters, integration_voicebank):
    ...
    - LLM 返回可解析 JSON
    - 输出与输入 segments 1:1
    - 同 speaker（小松鼠 在 seg_002 + seg_005）拿到相同 model
    ...
    # 同 speaker 一致性
    by_id = {inst.segment_id: inst for inst in result}
    if "seg_002" in by_id and "seg_005" in by_id:
        assert by_id["seg_002"].model == by_id["seg_005"].model, (
            f"小松鼠 跨段应用相同 model，实际 "
            f"{by_id['seg_002'].model} vs {by_id['seg_005'].model}"
        )
...
git commit -m "test(tts_director): C1 step 7 — 真 Gemma4 集成测试
...
- 1:1 与输入 segments 对应
- 同 speaker（小松鼠 seg_002 + seg_005）model 一致
- 不全选 default model（LLM 实际在做差异化决策）
...
### 任务 7：cosyvoice_http adapter 双接口
...
### 任务 9：indextts_http adapter 双接口
...
2. 把现有 `synthesize` 方法体重命名为 `_synthesize_legacy`。
3. 加 `synthesize` dispatcher（模式同任务 7-8）。
...
### 任务 11：registry.create_adapter_for_backend + lazy cache
...
### 任务 12：yaml_utils use_tts_director 开关识别
...
配合任务 14 实现的 CLI flag --use-tts-director 实现 OR 逻辑。
...
### 任务 13：pipeline stage 7 + stage 8 集成
...
### 任务 14：CLI flag --use-tts-director
...
### 任务 15：更新 CLAUDE.md / README.md / src_next_*.md
...
| C1 | 任务 1-6 | 5 新 + 0 改 | 12 unit + 1 integration |
| C2 | 任务 7-9 | 0 新 + 3 改 + 1 测试 | 6 unit |
| C3 | 任务 10-14 | 2 新 + 4 改 + 4 测试 | 17 unit + 2 smoke |
| C4 | 任务 15 | 0 新 + 4 改 | 0 |
| 验证 | 任务 16 | 0 | 0 |

**总计：16 任务，~25 文件，~38 测试**
