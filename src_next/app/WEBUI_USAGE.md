...
1. **缺块过滤**：YAML 必须包含完整 5 个顶层块：`llm` / `voicebank` / `tts` / `output` / `pipeline`。任一块缺失则**不显示**（如只配 `tts` 单块的 `blue_indextts.yaml` 会被过滤）；
2. **webui.enabled 过滤**：YAML 顶层若有 `webui.enabled: false`，则**不显示**（默认 `true`）；
3. **Malformed YAML**：语法错误的不显示，stderr 打 warning。
...
- **结果信息 bar** — 灰色信息条，包含：
  ...
  - **音频文件**名（如 `小红帽.wav`）
- **日志摘要** — 自动提取含 `[Summary]` / `success=` / `total_time` 等关键字的行。
